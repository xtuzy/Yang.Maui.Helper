...
#if ANDROID
using SKPlatformView = SkiaSharp.Views.Android.SKGLTextureView;
using SKNativePaintGLSurfaceEventArgs = SkiaSharp.Views.Android.SKPaintGLSurfaceEventArgs;
#else
using SKPlatformView = SkiaSharp.Views.iOS.SKMetalView;
using SKNativePaintGLSurfaceEventArgs = SkiaSharp.Views.iOS.SKPaintMetalSurfaceEventArgs;
#endif
namespace Yang.Maui.Helper.Skia.SKGpuView.Handlers
{
	public partial class SKMetalViewHandler: ViewHandler<SKMauiView, SKPlatformView>
	{
        ...
        private static void MapHasRenderLoop(SKMetalViewHandler handler, SKGpuView view)
        {
            handler.virtualView = view as ISKGpuView;
            handler.SetupRenderLoop(false, view);
        }

        private static void MapEnableTouchEvents(SKMetalViewHandler handler, SKGpuView view)
        {
            handler.touchHandler?.SetEnabled(handler.PlatformView, view.EnableTouchEvents);
        }

        private static void MapEnableTransparent(SKMetalViewHandler handler, SKGpuView view)
        {
        ...
        private static void MapGetGRContext(SKMetalViewHandler handler, SKGpuView view, object arg3)
        {
        ...
        private static void MapGetCanvasSize(SKMetalViewHandler handler, SKGpuView view, object arg3)
        {
        ...
        private static void MapSurfaceInvalidated(SKMetalViewHandler handler, SKGpuView view, object arg3)
        {
